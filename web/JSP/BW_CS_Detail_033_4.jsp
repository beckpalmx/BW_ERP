...
<%
            ...
            DataBeanD_product_report_daily_detail_prd objdb = new DataBeanD_product_report_daily_detail_prd();
            D_product_report_daily_detail_prdDAO objex = new D_product_report_daily_detail_prdDAO();
            ...
            if (request.getParameter("I_status") != null) {
                if (uti.numRowdatabase("Select Count(doc_id) As num from  d_product_report_daily_detail_prd  where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") != 0) {
                    statusck = -1;
                } else {
                    statusck = 1;
                    statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_product_report_daily_detail_prd  where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'");
            ...
            switch (statusck) {
                ...
                case 3:
                    if(request.getParameter("ckbox") != null && !request.getParameter("ckbox").equals("")){
                        objex.Delete_line_no(request.getParameter("ckbox"), objdb, "d_product_report_daily_detail_prd");
                    }else if(request.getParameter("chk_all") != null){
                        objex.Delete_all_line_no(objdb, "d_product_report_daily_detail_prd");
                    }
                    break;
                case 4:
                    break;
