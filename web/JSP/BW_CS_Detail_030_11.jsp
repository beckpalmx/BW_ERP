...
<%
            ...
            DataBeanD_product_roast_detail_1 objdb = new DataBeanD_product_roast_detail_1();
            D_product_roast_detail_1DAO objex = new D_product_roast_detail_1DAO();
            ...
            if (request.getParameter("I_status") != null) {
                if (uti.numRowdatabase("Select Count(doc_id) As num from  d_product_roast_detail_1 where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") != 0) {
                    statusck = -1;
                } else {
                    statusck = 1;
                    statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_product_roast_detail_1 where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'");
            ...
            if (statusck == 1 || statusck == 2) {
                ...
                objdb.setRoast_hr(request.getParameter(request_String_name[i]) == null ? "" :en.EncodeTexttoTIS(request.getParameter(request_String_name[i]).trim()));i+=1;
            ...
            switch (statusck) {
                ...
                case 3:
                    if(request.getParameter("ckbox") != null && !request.getParameter("ckbox").equals("")){
                        objex.Delete_line_no(request.getParameter("ckbox"), objdb, "d_product_roast_detail_1");
                    }else if(request.getParameter("chk_all") != null){
                        objex.Delete_all_line_no(objdb, "d_product_roast_detail_1");
                    }
                    break;
                case 4:
                    break;
