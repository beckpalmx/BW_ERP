...
<%
    ...
    ThaiUtil en = new ThaiUtil();
            ...
            if (request.getParameter("D_status") == null && request.getParameter("status_runid") == null) {
                statusnum = uti.numRowdatabase("SELECT Count(doc_id) As num from  d_product_accept_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'");
                if (statusnum == 0) {
                    if (uti.numRowdatabase("SELECT Count(doc_id) As num from  d_product_accept_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") == 1) {
                        statusck = -1;
                    } else {
                        statusck = 1;
                    }
            ...
            switch (statusck) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    out.print(objdb.getDoc_id().equalsIgnoreCase("")?new Runer_Doc_id().Run_id("d_product_accept_header"):new Runer_Doc_id().Run_id("d_product_accept_header",objdb.getDoc_id()));
                    break;
                default:
