...
<%
            ...
            DataBeanD_product_pack_detail_2 objdb = new DataBeanD_product_pack_detail_2();
            D_product_pack_detail_2DAO objex = new D_product_pack_detail_2DAO();
            ...
            if (request.getParameter("I_status") != null) {
                if (uti.numRowdatabase("Select Count(doc_id) As num from  d_product_pack_detail_3 where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and doc_line_no = '"+en.EncodeTexttoTIS(request.getParameter("A_doc_line_no").trim())+"' and delete_flag = 'N' and complete_flag = 'Y'") != 0) {
                    statusck = -1;
                } else {
                    statusck = 1;
                    statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_product_pack_detail_3 where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("A_doc_id").trim()) + "' and doc_line_no = '"+en.EncodeTexttoTIS(request.getParameter("A_doc_line_no").trim())+"' and delete_flag = 'N' and complete_flag = 'N'");
            ...
            if (statusck == 1 || statusck == 2) {
                ...
                objdb.setDoc_line_no(request.getParameter(request_String_name[i]) == null ? "" :en.EncodeTexttoTIS(request.getParameter(request_String_name[i]).trim()));i+=1;
                objdb.setProd_id(request.getParameter(request_String_name[i]) == null ? "" :en.EncodeTexttoTIS(request.getParameter(request_String_name[i]).trim()));i+=1;
            ...
            switch (statusck) {
                case -1:
                    ...
                    break;
                case 1:
                    objex.insert(objdb,"d_product_pack_detail_3");
                    out.print("บันทึกสำเร็จ");
                    break;
                case 2:
                    objex.update(objdb,"d_product_pack_detail_3");
                    out.print("แก้ไขสำเร็จ");
                    break;
                case 3:
                    if(request.getParameter("ckbox") != null && !request.getParameter("ckbox").equals("")){
                        objex.Delete_line_no(request.getParameter("ckbox"), objdb, "d_product_pack_detail_3");
                    }else if(request.getParameter("chk_all") != null){
                        objex.Delete_all_line_no(objdb, "d_product_pack_detail_3");
                    }
                    break;
                case 4:
                    break;
