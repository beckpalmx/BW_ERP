...
<%!    UtiDatabase uti = new UtiDatabase();
    ...
    Timestamp ts;
    DataBeanD_ticketsale objdb = new DataBeanD_ticketsale();
    VD_Casava_TicketDAO objex = new VD_Casava_TicketDAO();
...
<%
            ...
            if (request.getParameter("status_a") != null) {
                if (uti.numRowdatabase("Select Count(doc_id) As num from  d_ticketsale where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") != 0) {
                    statusck = -1;
                } else {
                    statusck = 1;
                    statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_ticketsale where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'Y'");
            ...
            switch (statusck) {
                ...
                case 2:
                    if (objex.update(objdb) != -1) {
                        ...
                        System.out.println("Edit Not OK ");
                        out.print("ไม่สามารถบันทึกได้");
                    }
                    System.out.println("objex.update(objdb) = " + objex.update(objdb));
                    break;
                case 3:
