<%@page import="com.bw.DB.D_Casava_Skin_doc_DAO"%>
<%@page import="com.bw.bean.DataBeanD_Casava_doc"%>
...
<%!    UtiDatabase uti = new UtiDatabase();
    ...
    DataBeanD_Casava_doc objdb = new DataBeanD_Casava_doc();
    D_Casava_Skin_doc_DAO objex = new D_Casava_Skin_doc_DAO();
    Timestamp ts;
...
<%
            ...
            if (request.getParameter("status_d") == null && request.getParameter("runer_id") == null) {
                statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_casava_skin_doc where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'Y'");
                if (statusnum == 0) {
                    if (uti.numRowdatabase("Select Count(doc_id) As num from  d_casava_skin_doc where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") == 1) {
                        statusck = -1;
                    } else {
                        statusck = 1;
                    }
            ...
            switch (statusck) {
                ...
                case 4:
                    ...
                    out.print(objrun.Run_id("d_casava_skin_doc", "d_adjust_rawmat_detail"));
                    break;
                default:
