...
<body>
    <%
                ...
                select_search_value = request.getParameter("select_search_value") == null ? memcoloumShow[1] : request.getParameter("select_search_value");
    %>
    <form name="serForm" id="serForm" method="get" action="Search_Config3.jsp">
                    ...
                    <td  colspan="2" class="fcentercenter">
                        <div style="width: 500px;" align="center">
                            <table width="550" cellpadding="0" cellspacing="0" border="0">
                                <tr>
                                    <td width="90"><input type="button" name="bt1" value="ย้อนกลับ" onClick="javascript:window.close();"/></td>
                                    ...
                                    <td width="199">
                                        <input type="button"  value="ค้นหา" onclick="EncodeBefor_Search()">
                                    </td>
        ...
        <div align="center">
            <%
                                                                                                if (screen > 1) {
                                                                                                    if (request.getParameter("searchid") == null) {%>
            <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=screen - 1%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>">ย้อนกลับ</a>
            <%
                                                                                                                } else if (request.getParameter("searchid") != null) {%>
            <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=screen - 1%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&searchid=<%=request.getParameter("searchid")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>">ย้อนกลับ</a>
            <%
                                                                                                    }
                                                                                                }
            ...
            <%
                                                                                                                        ...
                                                                                                                        if (request.getParameter("searchid") == null) {%>
		| <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=i%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>"><%=i%></a> |
            <%
                                                                                                                                    } else if (request.getParameter("searchid") != null) {%>
		| <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=i%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&searchid=<%=request.getParameter("searchid")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>"><%=i%></a> |
            <%
                                                                                                                        }
                                                                                                                    }
            ...
            <%
                                                                                                                                    ...
                                                                                                                                    if (request.getParameter("searchid") == null) {%>
			| <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=y%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>"><%=y%></a> |
            <%
                                                                                                                                                } else if (request.getParameter("searchid") != null) {%>
			| <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=y%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&searchid=<%=request.getParameter("searchid")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>"><%=y%></a> |
            <%
                                                                                                            }
                                                                                                        }
                                                                                                    }
                                                                                                ...
                                                                                                if (screen < total_page) {
                                                                                                    if (request.getParameter("searchid") == null ) {%>
            <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=screen + 1%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>">ไปข้างหน้า</a>
            <%
                                                                                                                } else if (request.getParameter("searchid") != null ) {%>
            <a href="../SEARCHSCREEN/Search_Config3.jsp?screen=<%=screen + 1%>&page=<%=request.getParameter("page")%>&table=<%=request.getParameter("table")%>&searchid=<%=request.getParameter("searchid")%>&stadment=<%=stadment%>&select_search_value=<%=select_search_value%>">ไปข้างหน้า</a>
            <%
                                                                                                                } 
                                } 
                            } catch (Exception e) {
