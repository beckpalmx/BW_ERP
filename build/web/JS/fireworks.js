...
piece.prototype.setC = function(colour) {
    if (nsDOM)
        this.elem.bgColor = colour;
    else
        this.style.backgroundColor = colour == null ? 'transparent' : colour;
};
...
fo = function(numst) {
    ...
    for (i = 0; i < numst; i++) {
        ...
        if (nsDOM) {
            ...
            this.sp[i].style.clip = "rect(" + 0 + " " + 3 + " " + 3 + " " + 0 + ")";
...
fo.prototype.animate = function() {
    ...
    if (this.step == 0) {
        ...
        for (i = 0; i < this.sp.length; i++)
            this.sp[i].fire(x, y, fw);
    }
    this.step++;
    for (i = 0; i < this.sp.length; i++)
        this.sp[i].animate(this.step, this.cl);
};
function stopfire() {
...
function fireworks() {
    ...
    if (stopafter > 0) {
        setTimeout('stopfire()', stopafter * 600000);
    }
}
