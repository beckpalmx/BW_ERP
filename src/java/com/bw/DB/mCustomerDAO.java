...
import java.sql.*;
import com.bw.bean.DataBeanCus_Sup;
...
public class mCustomerDAO {
    ...
    public int insert(DataBeanCus_Sup dataBean) throws Exception {
        ...
        String sql = "insert into mcustomer (customer_id,name_t,address1_t,tambol_code,amp_code,prov_code,zipcode,tel,fax,contact_name,remark,create_date,country_code,initial) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            p.setString(14, dataBean.getInitial());
    ...
    public int update(DataBeanCus_Sup dataBean) throws Exception {
        ...
        String sql =
                "update mcustomer set  name_t=?,address1_t=?,tambol_code=?,amp_code=?,prov_code=?,zipcode=?,tel=?,fax=?,contact_name=?,remark=?,update_date=?,country_code=?,initial=? where customer_id=? AND delete_flag <> 'Y'";
        PreparedStatement p = null;
        try {
            p = con.prepareStatement(sql);
            ...
            p.setString(13, dataBean.getInitial());
            p.setString(14, dataBean.getCustomer_id());
            return p.executeUpdate();
