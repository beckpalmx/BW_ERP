 ...
 * @author 
 */
public class VD_Casava_SkinDAO {
    ...
    /*    
     ...
     String sql = "insert into d_casava_skin (doc_id,line_no,product_id,location_id,weight,create_date,create_by,wh_in,wh_weight_final,wh_product_id,wh_warehouse_id,wh_location_id) "
     + "values(?,?,?,?,?,?,?,?,?,?,?,?)";        
    ...
    public int update(DataBeanD_ticketsale dataBean) throws Exception {
        ...
        String sql
                = "update d_casava_skin set  price_unit=?,price_total=?,update_date=?,update_by=? "
    ...
    public DataBeanD_ticketsale ReturnEdit(String Doc_id, String Ticket_text, DataBeanD_ticketsale bean) throws Exception {
        ...
        Connection con = dbConnect.openNewConnection();
        String SQL = "select * "
                + " from d_casava_skin  where doc_id = '" + Doc_id + "' and ticket_text ='" + Ticket_text + "' and delete_flag = 'N' ";
        //+ "and complete_flag = 'N' ";
        ...
        try {
            if (inputarry.length() != 0) {
                ...
                for (int i = 0; i < strarry.length; i++) {
                    ...
                    sql = "update d_casava_skin set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
                    p = con.prepareStatement(sql);
                    p.setString(1, "Y");
                ...
                if (strarry.length != -1) {
                    sql = "select count(doc_id) as num from d_casava_skin where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag = 'N'";
                    if (objuti.numRowdatabase(sql) != 0) {
                        p = null;
                        sql2 = "Select line_no from  d_casava_skin where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag ='N' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_casava_skin set line_no=? where doc_id=? and line_no=?";
                            p = con.prepareStatement(sql);
    ...
    public void Delete_line_no_All(DataBeanD_ticketsale dataBean) throws Exception {
        ...
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sqldetail = "update d_casava_skin set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag = 'N' and complete_flag = 'N'";
        PreparedStatement p = null;
        try {
...
//        SQL1 = "Select Count(doc_id) As num from  d_casava_skin where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' and complete_flag ='N' ";
          SQL1 = "Select Count(doc_id) As num from  d_casava_skin where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' ";
        int a = objuti.numRowdatabase(SQL1);
        strOutput += "<table width='100%' align='center' border='0' cellpadding='0' cellspacing='1' class='inner'>";
        SQL = "select * "
                + " from d_casava_skin  where doc_id = '" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' "  
                + " and (prod_code = '03' or prod_code = '04') "
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('ticket_text_" + rs.getString("ticket_text") + ",username,doc_id','../SCREEN/CS_D_Casava_Skin_2.jsp'))\"><input type='hidden' id='ticket_text_" + rs.getString("ticket_text") + "' value=\"" + rs.getString("ticket_text") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("ticket_text") + "</a></td>";
