...
import com.bw.bean.DataBeanMproduction_place;
...
public class Mproduction_placeDAO {
    ...
    public int insert(DataBeanMproduction_place dataBean) throws Exception {
        ...
        String sql = "INSERT INTO mproduction_place(prd_place,name_t,remark,create_date,create_by) values(?,?,?,?,?)";
    ...
    public int  update(DataBeanMproduction_place dataBean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sql = "UPDATE mproduction_place SET name_t=?,remark=?,update_date=?,update_by=? where prd_place = ? and delete_flag <> 'Y'";
        PreparedStatement p = null;
        ...
        try {
            ...
            p.setString(5, dataBean.getPrd_place());
            returnvalue = p.executeUpdate();
        } finally {
            try {
                p.close();
