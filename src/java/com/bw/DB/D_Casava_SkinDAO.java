...
//        SQL1 = "Select Count(doc_id) As num from  d_casava_skin where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' and complete_flag ='N' ";
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('ticket_text_" + rs.getString("ticket_text") + ",username,doc_id','../SCREEN/CS_D_Casava_Skin_1.jsp'))\"><input type='hidden' id='ticket_text_" + rs.getString("ticket_text") + "' value=\"" + rs.getString("ticket_text") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("ticket_text") + "</a></td>";
