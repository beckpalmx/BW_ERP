...
import com.bw.bean.DataBeanD_ticketbuy;
...
public class D_ticketbuyDAO {
    ...
    /*    
     ...
     String sql = "insert into d_wood_fuel (doc_id,line_no,product_id,location_id,weight,create_date,create_by,wh_in,wh_weight_final,wh_product_id,wh_warehouse_id,wh_location_id) "
     + "values(?,?,?,?,?,?,?,?,?,?,?,?)";        
    ...
    public int update(DataBeanD_ticketbuy dataBean) throws Exception {
        ...
        String sql
                = "update d_wood_fuel set  price_unit=?,price_total=?,update_date=?,update_by=? "
    ...
    public DataBeanD_ticketbuy ReturnEdit(String Doc_id, String Ticket_text, DataBeanD_ticketbuy bean) throws Exception {
        ...
        Connection con = dbConnect.openNewConnection();
        String SQL = "select * "
                + " from d_wood_fuel  where doc_id = '" + Doc_id + "' and ticket_text ='" + Ticket_text + "' and delete_flag = 'N' ";
        //+ "and complete_flag = 'N' ";
        ...
        try {
            if (inputarry.length() != 0) {
                ...
                for (int i = 0; i < strarry.length; i++) {
                    ...
                    sql = "update d_wood_fuel set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
                    p = con.prepareStatement(sql);
                    p.setString(1, "Y");
                ...
                if (strarry.length != -1) {
                    sql = "select count(doc_id) as num from d_wood_fuel where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag = 'N'";
                    if (objuti.numRowdatabase(sql) != 0) {
                        p = null;
                        sql2 = "Select line_no from  d_wood_fuel where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag ='N' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_wood_fuel set line_no=? where doc_id=? and line_no=?";
                            p = con.prepareStatement(sql);
    ...
    public void Delete_line_no_All(DataBeanD_ticketbuy dataBean) throws Exception {
        ...
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sqldetail = "update d_wood_fuel set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag = 'N' and complete_flag = 'N'";
        PreparedStatement p = null;
        try {
...
//        SQL1 = "Select Count(doc_id) As num from  d_wood_fuel where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' and complete_flag ='N' ";
          SQL1 = "Select Count(doc_id) As num from  d_wood_fuel where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' ";
        int a = objuti.numRowdatabase(SQL1);
        strOutput += "<table width='100%' align='center' border='0' cellpadding='0' cellspacing='1' class='inner'>";
        SQL = "select * "
                + " from d_wood_fuel  where doc_id = '" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' "                
