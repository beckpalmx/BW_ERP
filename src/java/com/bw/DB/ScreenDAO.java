...
public class ScreenDAO {
    public boolean insert(ScreenBean scBean) throws Exception {
        ...
        String sql = "INSERT INTO mscreen(screen_id,process_id,name_t,dept_id,qc_check,iso_no,effective_date,remark,create_date) values(?,?,?,?,?,?,?,?,?)";
    ...
    public void  update(ScreenBean scBean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sql = "UPDATE mscreen SET process_id=?,name_t=?,dept_id=?,qc_check=?,iso_no=?,effective_date=?,remark=?,update_date=? where screen_id = ?";
        PreparedStatement p = null;
        ...
        try {
            ...
            p.setString(9, scBean.getScreen_id());
            p.executeUpdate();
        } finally {
            try {
                p.close();
