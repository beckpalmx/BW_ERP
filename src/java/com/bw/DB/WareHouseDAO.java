...
import com.bw.bean.WarehouseBean;
...
public class WareHouseDAO {
    public boolean insert(WarehouseBean whBean) throws Exception {
        ...
        String sql = "INSERT INTO mwarehouse(warehouse_id,name_t,remark,create_date) values(?,?,?,?)";
    ...
    public void  update(WarehouseBean whBean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sql = "UPDATE mwarehouse SET name_t=?,remark=?,update_date=? where warehouse_id = ?";
        PreparedStatement p = null;
        ...
        try {
            ...
            p.setString(4, whBean.getWarehouse_id());
            p.executeUpdate();
        } finally {
            try {
                p.close();
