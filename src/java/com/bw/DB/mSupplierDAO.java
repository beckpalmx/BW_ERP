...
public class mSupplierDAO {
    ...
    public int insert(DataBeanCus_Sup dataBean) throws Exception {
        ...
        String sql = "insert into msupplier (supplier_id,name_t,address1_t,tambol_code,amp_code,prov_code,zipcode,tel,fax,contact_name,remark,create_date) values(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int Update(DataBeanCus_Sup dataBean) throws Exception {
        ...
        Connection con = dbConnect.openNewConnection();
        String sql =
                "update msupplier set  name_t=?,address1_t=?,tambol_code=?,amp_code=?,prov_code=?,zipcode=?,tel=?,fax=?,contact_name=?,remark=?,update_date=? where supplier_id=? AND delete_flag <> 'Y'";
        PreparedStatement p = null;
        try {
            p = con.prepareStatement(sql);
            ...
            p.setString(12, dataBean.getSupplier_id());
