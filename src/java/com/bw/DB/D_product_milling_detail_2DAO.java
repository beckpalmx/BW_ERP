...
import com.bw.bean.DataBeanD_product_milling_detail_2;
 ...
 * @author B
 */
public class D_product_milling_detail_2DAO {
    private StringQuery obj_StringQuery;

    public void insert(DataBeanD_product_milling_detail_2 DataBean) throws Exception {
        ...
        int i = 1;
        try {
            p = con.prepareStatement(obj_StringQuery.Insert_String(String_input, "d_product_milling_detail_2"));
            ...
            p.setString(i, DataBean.getTotal_time());i+=1;
    ...
    public void update(DataBeanD_product_milling_detail_2 DataBean) throws Exception {
        ...
        int i = 1;
        try {
            p = con.prepareStatement(obj_StringQuery.Update_String(String_update, "d_product_milling_detail_2",
    ...
    public void Delete_line_no(String Select_line_no, DataBeanD_product_milling_detail_2 DataBean, String Table_Detail) throws Exception {
    ...
    public void Delete_all_line_no(DataBeanD_product_milling_detail_2 DataBean, String Table_Detail) throws Exception {
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        obj_StringQuery = new StringQuery();
        PreparedStatement p = null;
    ...
    public DataBeanD_product_milling_detail_2 Return_Edit(String doc_id,String line_no)throws Exception{
        DataBeanD_product_milling_detail_2 objbean = new DataBeanD_product_milling_detail_2();
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        ResultSet rs = null;
        String SQL = "SELECT "
                + "date_from,time_from,date_to,time_to,total_time,remark"
                + " FROM d_product_milling_detail_2 "
                + " WHERE doc_id = '"+doc_id+"' and line_no = '"+line_no+"' and delete_flag = 'N' and complete_flag = 'N'";
        try{
            rs = con.createStatement().executeQuery(SQL);
            while(rs.next()){
                objbean.setDate_from(rs.getString("date_from"));
                objbean.setTime_from(rs.getString("time_from"));
                objbean.setDate_to(rs.getString("date_to"));
                objbean.setTime_to(rs.getString("time_to"));
                objbean.setTotal_time(rs.getString("total_time"));
                objbean.setRemark(rs.getString("reamrk"));
    ...
    public String Show_Detail(String doc_id, String Screen) throws Exception {
        ...
        ResultSet rs = null;
        String SQL = "";
        total = objuti.numRowdatabase("Select Count(doc_id) As num from  d_product_milling_detail_2 where doc_id ='" + doc_id + "' and delete_flag = 'N' and complete_flag ='N' ");
        try {
            ...
            if (total != 0) {
                ...
                if (screen >= 1 && screen <= total) {
                    ...
                    SQL = "select "
                            + "runno,doc_id,line_no,date_from,time_from,date_to,time_to,total_time,remark "
                            + "from d_product_milling_detail_2 "
                    ...
                    while (rs.next()) {
                        ...
                        Return_String.append("<td class='forborder' width='3px'><input type='checkbox' id='ckbox2_"+rs.getString("line_no")+"' value='"+rs.getString("line_no")+"' name='ckbox2' onclick=\"cancle_chkboxAll('chk_all2',this.checked)\"></td>");
                        Return_String.append("<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",A_doc_id','../SCREEN/BW_CS_Detail_030_32.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a></td>\n");
                        Return_String.append("<td class='forborder' width='15%'>" + objuti.NotNull(rs.getString("date_from")) + "&nbsp;</td>\n");
                        Return_String.append("<td class='forborder' width='15%'>" + objuti.NotNull(rs.getString("time_from")) + "</td>\n");
                        Return_String.append("<td class='forborder' width='15%'>" + objuti.NotNull(rs.getString("date_to")) + "</td>\n");
                        Return_String.append("<td class='forborder' width='15%'>" + objuti.NotNull(rs.getString("time_to")) + "</td>\n");
                        Return_String.append("<td class='forborder' width='15%'>" + objuti.NotNull(rs.getString("total_time")) + "</td>\n");
                ...
                if ((screen % 10) != 0 && screen <= 10) {
                    for (int count = 1; count <= 10 && count <= total_page; count++) {
                        if (count == screen) {
                            ...
                            Return_String.append(" <a href='#'  onclick = \"showTableNonPopup_Detail('show2','A_doc_id','s_screen_"+count+"','../SHOWDETAILSCREEN/BW_CS_Show030_32.jsp?doc_id=')\"><input type = 'hidden' id='s_screen_"+count+"' name='s_screen_"+count+"' value =\""+count+"\">" + count + "</a> |");
                        }
                    ...
                    for(int count = 1; count <= 10 && count <= total_page; count++){
                        if (count == screen) {
                            Return_String.append("<option selected  value=\'"+count+"\' onclick = \"showTableNonPopup_Detail('show2','A_doc_id','select_screen','../SHOWDETAILSCREEN/BW_CS_Show030_32.jsp?doc_id=')\">"+count+"</option>");
                        }else{
                            Return_String.append("<option   value=\'"+count+"\' onclick = \"showTableNonPopup_Detail('show2','A_doc_id','select_screen','../SHOWDETAILSCREEN/BW_CS_Show030_32.jsp?doc_id=')\">"+count+"</option>");
                        }
                    }
