...
import com.bw.bean.DataBeanD_product_reprot_daily_detail_raw;
 ...
 */
public class D_product_report_daily_detail_rawDAO {
    public void insert(DataBeanD_product_reprot_daily_detail_raw DataBean)throws Exception{
        ...
        int i = 1;
        try{
            p = con.prepareStatement(objStringQuery.Insert_String(String_input, "d_product_report_daily_detail_raw"));
    ...
    public void update(DataBeanD_product_reprot_daily_detail_raw DataBean)throws Exception{
        String[] String_input = new String[]{
            "product_id",
            "weight",
            "bag_qty",
        ...
        int i = 1;
        try{
            p = con.prepareStatement(objStringQuery.Update_String(String_input, "d_product_report_daily_detail_raw",
    ...
    public void Delete_line_no(String Select_line_no, DataBeanD_product_reprot_daily_detail_raw DataBean, String Table_Detail) throws Exception {
    ...
    public void Delete_all_line_no(DataBeanD_product_reprot_daily_detail_raw DataBean, String Table_Detail) throws Exception {
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        StringQuery objStringQuery = new StringQuery();
        PreparedStatement p = null;
    ...
    public DataBeanD_product_reprot_daily_detail_raw Return_Edit(String doc_id,String line_no)throws Exception{
        DataBeanD_product_reprot_daily_detail_raw objbean = new DataBeanD_product_reprot_daily_detail_raw();
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        ResultSet rs = null;
        String SQL = "SELECT "
                + "product_id,weight,bag_qty,amount,remark"
                + " FROM d_product_report_daily_detail_raw "
            ...
            if (total != 0) {
                ...
                if (screen >= 1 && screen <= total) {
                    ...
                    while (rs.next()) {
                        ...
                        Return_String.append("<td class='forborder' width='3px'><input type='checkbox' id='ckbox_3_"+rs.getString("line_no")+"' value='"+rs.getString("line_no")+"' name='ckbox_3' onclick=\"cancle_chkboxAll('chk_all3',this.checked)\"></td>");
                        Return_String.append("<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",A_doc_id','../SCREEN/BW_CS_Detail_033_3.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a></td>\n");
                ...
                if ((screen % 10) != 0 && screen <= 10) {
                    for (int count = 1; count <= 10 && count <= total_page; count++) {
                        if (count == screen) {
                            ...
                            Return_String.append(" <a href='#'  onclick = \"showTableNonPopup_Detail('show2','A_doc_id','s_screen_"+count+"','../SHOWDETAILSCREEN/BW_CS_Show033_3.jsp?doc_id=')\"><input type = 'hidden' id='s_screen_"+count+"' name='s_screen_"+count+"' value =\""+count+"\">" + count + "</a> |");
                        }
                    ...
                    for(int count = 1; count <= 10 && count <= total_page; count++){
                        if (count == screen) {
                            Return_String.append("<option selected  value=\'"+count+"\' onclick = \"showTableNonPopup_Detail('show2','A_doc_id','select_screen','../SHOWDETAILSCREEN/BW_CS_Show033_3.jsp?doc_id=')\">"+count+"</option>");
                        }else{
                            Return_String.append("<option   value=\'"+count+"\' onclick = \"showTableNonPopup_Detail('show2','A_doc_id','select_screen','../SHOWDETAILSCREEN/BW_CS_Show033_3.jsp?doc_id=')\">"+count+"</option>");
                        }
                    }
