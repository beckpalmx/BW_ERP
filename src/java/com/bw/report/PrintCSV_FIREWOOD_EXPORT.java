...
public class PrintCSV_FIREWOOD_EXPORT {
    ...
    public void ExportCSV(HttpServletRequest request, HttpServletResponse response) throws IOException, Exception {
        String Str_Writer = "", SQL = null, cred_code = "";
        ...
        try {
            ...
            if (request.getParameter("supplier_id") != null && !request.getParameter("supplier_id").equals("")) {
                SQL += SQL.indexOf("WHERE") != -1 ? " AND cred_code = '" + request.getParameter("supplier_id") + "' " : " WHERE cred_code = '" + request.getParameter("supplier_id") + "' ";
            }

            SQL += " order by cred_code,to_date(format_date(doc_date),'YYYY-MM-DD')";
            ...
            pw.println("วันที่,เลขที่ใบชั่ง,ทะเบียนรถ,ชื่อผู้ขาย,ประเภท,น้ำหนัก,ราคาต่อหน่วย,ราคารวม\n");
            ...
            while (rs.next()) {
                ...
                //Str_Writer = null;
                i++;
                cred_code = rs.getString("cred_code");
            }
